...
- Data staat voorberekend in `public/data/*.json`; de browser rekent alles zelf (geen routing-API). Scripts in `scripts/` maken de data: `fetch_osm.mjs` → `build_graph.py` → `refine_graph.py`, `fetch_pois.mjs` → `build_pois.py`. Wijzig het dataformaat alleen samen met `src/types.ts` en de build scripts.
- Routering leeft in `src/routing.ts` (A*, kostenfunctie `evalEdge` met `REALISM`, alternatieven, afslagen) en `src/graph.ts` (`classifyWay`: snelheidsplafond en voorkeursfactor per vaarweg). Nieuwe beperkingen of tijdsafhankelijke kosten horen in `evalEdge`, niet in de UI. Landroutes rekent de app niet zelf; `src/lastmile.ts` schat lopen en linkt naar een externe planner.
